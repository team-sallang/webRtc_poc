import { CONNECTION_LOCKED_TAG, Tags, WakuEvent } from "@waku/interfaces";
...
export class ConnectionLimiter {
    ...
    async maintainTTLConnectedPeers() {
        ...
        const promises = this.libp2p.getConnections().map(async (c) => {
            try {
                ...
                log.error(`Unexpected error while maintaining TTL connected peer`, error);
            }
        });
        await Promise.all(promises);
    }
    ...
    async triggerBootstrap() {
        ...
        log.info(`Found ${bootstrapComponents.length} bootstrap components, starting them`);
