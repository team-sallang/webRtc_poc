...
class MessageChannel extends TypedEventEmitter {
    ...
    sweepIncomingBuffer() {
        const { buffer, missing } = this.incomingBuffer.reduce(({ buffer, missing }, message) => {
            ...
            log.info(this.senderId, "message from incoming buffer", message.messageId, "is missing dependencies", missingDependencies.map(({ messageId, retrievalHint }) => {
    ...
    async pushOutgoingSyncMessage(callback) {
        ...
        if (callback) {
            try {
                ...
                log.error("Callback execution failed in pushOutgoingSyncMessage:", error);
                throw error;
            }
        }
        return false;
    ...
    _pushIncomingMessage(message) {
        ...
        log.info(`${this.senderId} incoming message ${message.messageId}`, `retrieval hint: ${bytesToHex(message.retrievalHint ?? new Uint8Array())}`);
        const isDuplicate = message.content &&
            message.content.length > 0 &&
            this.timeReceived.has(message.messageId);
        if (isDuplicate) {
            log.info(this.senderId, "dropping dupe incoming message", message.messageId);
            return;
        }
        ...
        if (!isSyncMessage(message) && !isContentMessage(message)) {
            log.error(this.senderId, "internal error, a message is neither sync nor ephemeral nor content, ignoring it", message);
            return;
        }
    ...
    async _pushOutgoingEphemeralMessage(payload, callback) {
        ...
        if (callback) {
            try {
                ...
                log.error("Callback execution failed in _pushOutgoingEphemeralMessage:", error);
                throw error;
            }
        }
    }
    ...
    deliverMessage(message) {
        ...
        log.info(this.senderId, "delivering message", message.messageId, message.retrievalHint);
    ...
    reviewAckStatus(receivedMessage) {
        log.info(this.senderId, "reviewing ack status using causal history:", receivedMessage.causalHistory.map((ch) => ch.messageId));
        log.info(this.senderId, "current outgoing buffer:", this.outgoingBuffer.map((b) => b.messageId));
        ...
        this.outgoingBuffer = this.outgoingBuffer.filter((message) => {
            ...
            if (count < this.possibleAcksThreshold) {
                ...
                log.info(this.senderId, "message possibly acknowledged", message.messageId, count);
...
export { BloomFilter, ContentMessage, EphemeralMessage, Message, MessageChannel, MessageChannelEvent, SyncMessage, isContentMessage, isEphemeralMessage, isSyncMessage };
